	...
	<body>
		...
		<div id="main" class="mui-clearfix">
			<div class="order-top clearfloat">
				<ul>
					<li class="clearfloat"><a href="<?php echo U('myOrder');?>">全部</a></li>
					<li class="clearfloat"><a href="<?php echo U('needPay');?>">待付款</a></li>
					<li class="clearfloat"><a href="<?php echo U('needSend');?>">待发货</a></li>
					<li class="clearfloat cur"><a href="<?php echo U('needGet');?>">待收货</a></li>
			...
			<?php if(is_array($order)): foreach($order as $k=>$v): ?><div class="order-list clearfloat">
					...
					<input type="hidden" value="<?php echo ($v["orderid"]); ?>" id="inp">
					<a class="gopay-btn fr inp">确认收货</a>
				</div><?php endforeach; endif; ?>
			<style type="text/css">
...
<script>
	$('.inp').click(function () {
		oid = $('#inp').val();
		$.get("<?php echo U('Center/trueGood');?>",{oid:oid},function () {
			//询问框
			layer.open({
				content: '已确认收货是否马上评价？'
				,btn: ['去评价', '取消']
				,yes: function(){
					location.href="<?php echo U('needSpeak');?>";
				}
				,no: function () {
					location.reload();
				}
